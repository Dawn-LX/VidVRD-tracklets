## version 6_3
# TrajProposal.traj_durations 和 VideoGraph.traj_durations 都转为 tensor 且都是闭区间
# VideoGraph.pred_durations 也转为闭区间
# 对于标注有间断的gt traj， 视为同一个entity
# traj proposal: MAX_PROPOSAL = 250
# gt: MAX_PRED = 250
import os
import json
...
class VideoGraph(object):
    def __init__(self,video_info,is_train,MAX_PREDS,
        ...
        if is_train and (self.num_preds > self.MAX_PREDS):
    ...
    def to_eval_format(self,enti_FG=False,pred_FG=False):
        ...
        for p_id in range(self.num_preds):
            ...
            # result_per_triplet["triplet"] = [vidvrd_CatId2name[subject_catid],vidvrd_PredId2name[pred_catid],vidvrd_CatId2name[object_catid]]
...
class Dataset(object):
    def __init__(self,dataset_type,ann_dir,train_proposal_dir,test_proposal_dir,max_proposal,max_preds,score_th,
        dim_boxfeature,min_frames_th,is_train=True,in_memory_mode=True,train_gttrajs_path = None
    ):
        self.max_proposal = max_proposal
        self.max_preds = max_preds
        ...
        if self.dataset_type.lower() == "vidvrd":
            self.enti_CatName2Id = vidvrd_CatName2Id
            self.pred_CatName2Id = vidvrd_PredName2Id
            if self.is_train:
        ...
        if self.is_train:
            self.proposal_dir = train_proposal_dir                          # e.g., "proposals/vidvrd-dataset/VidVRD_train_every1frames/"
        else:
            self.proposal_dir = test_proposal_dir
        ...
        self.train_gttrajs_path = train_gttrajs_path
        if self.is_train and (train_gttrajs_path!=None):
            print("use gt_trajs when training, load {}".format(train_gttrajs_path))
            with open(train_gttrajs_path,'rb') as f:
                self.gt_trajswithfeatures = pickle.load(f)
        ...
        if self.in_memory_mode:
            ...
            for idx,pr_di in enumerate(self.proposal_dir.values()):
                ...
                cache_file = "v6_3_" + temp[-2] + temp[-1] + "_th_{}-{}-{}-{:.2f}".format(self.min_frames_th,self.max_proposal,self.max_preds,self.score_th)
                cache_file = os.path.join(self.cache_dir,cache_file + ".pkl")

                if os.path.exists(cache_file):
                    print("loading data into memory..., from cache file {}".format(cache_file))
            ...
            ## filter_videos
            if self.dataset_type.lower() == "vidor" and self.is_train:
                frame_count_th = 2700
    ...
    def _prepare_video_names(self):
        if self.dataset_type.lower() == "vidvrd":
            video_name_list_all = os.listdir(self.video_ann_dir)  # e.g., "datasets/vidvrd-dataset/train"
            video_name_list_all = sorted([v.split('.')[0] for v in video_name_list_all])
        elif self.dataset_type.lower() == "vidor":
    ...
    def __getitem__(self,idx):
        ...
        if self.in_memory_mode:
            ...
            traj_proposal, gt_graph = self.get_data(video_name)
        ...
        if self.is_train:
            ...
            #     idx = random.randint(0,len(self.video_name_list)-1)
            #     print("video:{} gt_trajs={}, gt_preds={}".format(video_name,gt_graph.num_trajs,gt_graph.num_preds))
            #     return self.__getitem__(idx)
            # else:
            #     return traj_proposal, gt_graph
    ...
    def get_data(self,video_name):
        if self.is_train and (self.train_gttrajs_path != None):
            trajs_gt = self._get_gttraj(video_name)
        else:
            trajs_gt = None
        trajs_prop = self._get_proposal(video_name)
        traj_proposal = self.merge_trajs(trajs_prop,trajs_gt,video_name)
    ...
    def videoname2trackres(self,video_name):
        if self.is_train and self.dataset_type.lower() == "vidor":
    ...
    def _get_proposal(self,video_name):
        ...
        for tid in trajs.keys():
            ...
            trajs[tid]["box_feats"] = np.array(trajs[tid]["box_feats"])
    ...
    def videoname2annoname(self,video_name):
        # video_name: e.g., 0004_11566980553
        if self.dataset_type.lower() == "vidvrd":
            anno_name = video_name + ".json"
        elif self.dataset_type.lower() == "vidor":
            temp = video_name.split('_')
            anno_name = temp[0] + "/" + temp[1] + ".json"
        else:
            raise NotImplementedError
    ...
    def _get_gt_graph(self,video_name):
        ...
        anno_name = self.videoname2annoname(video_name) # e.g., "ILSVRC2015_train_00005003.json" or "0004/11566980553.json"
        ...
        return VideoGraph(video_info,self.is_train,self.max_preds,
