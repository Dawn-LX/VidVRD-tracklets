...
def eval_pred_only(gt_relations, pred_relations):
    pred_relations = sorted(pred_relations, key=lambda x: x['score'], reverse=True)
    # ignore trajectories
    gt_triplets = set(tuple(r['triplet'])[1] for r in gt_relations)
    pred_triplets = []
    hit_scores = []
    for r in pred_relations:
        triplet = tuple(r['triplet'])[1]
        if not triplet in pred_triplets:
            pred_triplets.append(triplet)
            hit_scores.append(r['score'])
    hit_scores = np.asarray(hit_scores)
...
def evaluate(groundtruth, prediction, viou_threshold=0.5,
        det_nreturns=[50, 100], tag_nreturns=[1, 5, 10],is_print=True,pred_only=False):
    ...
    for vid, gt_relations in tqdm(groundtruth.items()):
        ...
        if pred_only:
            tag_prec, _, _ = eval_pred_only(gt_relations, predict_relations)
        else:
