...
class MEGAFeatureExtractor(AttentionExtractor):
    ...
    def forward(self, x, proposals, pre_calculate=False):
        assert isinstance(proposals,list) #ChangeNote
        if pre_calculate:
            return self._forward_ref(x, proposals)
    ...
    def generate_feats_test(self, x, proposals):
        ...
        # 上面这些 proposal 开头的是box位置， x开头的是 feature map
    ...
    def _forward_ref(self, x, proposals):
        ...
        # x.shape == (N,2048,H/16,W/16)
        #DebudCode
            # idx = (x[0][0,:,15,15] > 0.01).nonzero(as_tuple=True)[0]
            # print(idx[:10],"idx[:10]")
            # print(x[0][0,idx[:10],15,15],x[0].shape)  # # 这个 cuda 和cpu 是一样的
            # 问题出在 RoIAlign
            # import numpy as np
            # np.save("test_gramma/before_RoIAlign_feature.npy",x[0].cpu().numpy())

        x = self.pooler(x, proposals)  # x.shape == (num_obj,2048,7,7)
    ...
    def _forward_test(self, x, proposals):
        # proposals:[proposals, proposals_ref, proposals_ref_dis, proposals_feat_ref, proposals_feat_ref_dis]
        # 第一个是 list， 
        # proposals_ref, proposals_ref_dis 是 BoxList
        ...
        if self.conv is not None:
            ...
            x = (self.head(x), )
        x = self.pooler(x, proposals[0])  # proposals[0] 是当前帧的 proposal
        x = x.flatten(start_dim=1)
        x = F.relu(self.l_fcs[0](x))
