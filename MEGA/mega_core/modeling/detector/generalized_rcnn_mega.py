    ...
    def _forward_test(self, imgs, infos, targets=None):
        ...
        if infos["frame_category"] == 0:  # a new video
            ...
            while len(self.feats) < self.all_frame_interval:
                ...
                # print(infos["transforms"],"infos[")
                end_image = infos["transforms"](end_image)  # 这里的 end_image 是一个 tensor ， # transforms 最后有 to tensor 的操作
            ...
            self.end_id = min(self.end_id + 1, self.seg_len - 1)
            # print(infos["ref_l"][0],type(infos["ref_l"][0]))
            end_image = infos["ref_l"][0].tensors  # ref_l 里面直接取出来的已经经过 transforms 了, 这里的[0]因为它是一个len==1的list
